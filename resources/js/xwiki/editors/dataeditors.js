...
C.stop();
if(!B.disabled){new XWiki.widgets.ConfirmedAjaxRequest(B.readAttribute("href"),{onCreate:function(){B.disabled=true
...
C.stop();
window.location=B.readAttribute("href")
}.bindAsEventListener())
});
...
}$$(".xproperty-title").each(function(B){var A=new Element("img",{src:"$xwiki.getSkinFile('icons/datamodel/move.png')","class":"move",alt:"move",title:"Drag and drop to change the order"});
B.makePositioned();
B.appendChild(A);
