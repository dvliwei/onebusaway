...
}XWiki.editors.AutoSave=Class.create({enabled:false,frequency:5,initialize:function(){if(!(this.form=$("xwikieditcontent"))||!(this.form=this.form.up("form"))){return 
...
}},createUIElements:function(){this.autosaveCheckbox=new Element("input",{type:"checkbox",checked:this.enabled});
this.autosaveInput=new Element("input",{type:"text",value:this.frequency,size:"2","class":"autosave-frequency"});
var B=new Element("label");
...
if(!this.enabled){C.addClassName("hidden")
}var A=new Element("p",{id:"autosaveControl"});
A.appendChild(B);
A.appendChild(document.createTextNode(" "));
A.appendChild(C);
$(document.body).down(".alleditcontent").insert({bottom:A});
